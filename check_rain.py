...
RAIN_THRESHOLD_MM = 0.1  # 降水量阈值
NOTIFY_METHOD = os.environ.get("NOTIFY_METHOD", "bark")
BARK_KEY = os.environ.get("BARK_KEY", "")
PUSHOVER_TOKEN = os.environ.get("PUSHOVER_TOKEN", "")
PUSHOVER_USER = os.environ.get("PUSHOVER_USER", "")
# GitHub Actions workflow_dispatch 的 test_mode 输入
...
def send_via_bark(message):
    """通过 Bark POST 方式推送（更稳定，避免 URL 编码问题）。"""
    if not BARK_KEY:
...
def send_via_bark_get_fallback(message):
    """GET 方式（备用）。"""
    title = "🌂 带伞提醒"
    url = f"https://api.day.app/{BARK_KEY}/{urllib.parse.quote(title)}/{urllib.parse.quote(message)}?isArchive=1&level=timeSensitive"
...
def send_via_pushover(message):
    if not PUSHOVER_TOKEN or not PUSHOVER_USER:
        print("❌ 缺失 PUSHOVER_TOKEN 或 PUSHOVER_USER", file=sys.stderr)
        sys.exit(1)
    data = urllib.parse.urlencode({
        "token": PUSHOVER_TOKEN, "user": PUSHOVER_USER,
        "title": "🌂 带伞提醒", "message": message,
        "sound": "persistent", "priority": 1,
    }).encode()
...
def send_notification(message):
    try:
        ...
        if NOTIFY_METHOD != "pushover" and BARK_KEY:
            print(f"⚠️ POST 失败，尝试 GET: {e}")
            return send_via_bark_get_fallback(message)
        raise
